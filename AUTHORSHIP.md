...
The [complete mapping](commit-authorship.json) records original commits, identities, inherited trailers, exclusions, old/new hashes and unchanged trees. These are newly assembled commits: original source commit IDs/signatures are not transplanted. Original PR histories remain untouched.
...
Only metadata changed. Every candidate retains its prior tree and base. Check receipts keep their actual tested commit IDs and are reused for the identical trees. Portable Git bundles include the tested old commits as audit refs for independent verification.
